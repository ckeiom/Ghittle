...
void hmp_nve( Monitor *mon, const QDict *qdict )
{
	FILE *in; FILE* out;
	char str[50];
	uint8_t buf[512];
	...
	monitor_printf(mon, "NVRAM emulator (to save current memory state)\n");

	
	in = fopen("nvimage.fmt","r");

	if( !in )
	{
		monitor_printf(mon, "\'nvimage.fmt\' does not exist" );
		return;
	}

	out = fopen("nvimage","w");

	while ( ptr = fgets( str, 50, in ) )
	{
		monitor_printf(mon, "%s",ptr );
		...
		while ( size > 0 )
		{
			...
			cpu_physical_memory_read( start, buf, size );
			start += size;
